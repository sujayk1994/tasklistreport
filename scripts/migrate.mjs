...
async function migrate() {
  ...
  try {
    ...
    // ── Twitter Marketing completion tracking ──────────────────────────────────
    await client.query(`
      CREATE TABLE IF NOT EXISTS twitter_marketing_completions (
        id                 SERIAL PRIMARY KEY,
        magazine           TEXT NOT NULL,
        project            TEXT NOT NULL,
        completed_at       TIMESTAMP NOT NULL DEFAULT NOW(),
        print_task_created BOOLEAN NOT NULL DEFAULT false,
        created_at         TIMESTAMP NOT NULL DEFAULT NOW()
      );
    `);

    // ── Address receipts (Copies Required email) ───────────────────────────────
    await client.query(`
      CREATE TABLE IF NOT EXISTS address_receipts (
        id          SERIAL PRIMARY KEY,
        magazine    TEXT NOT NULL,
        project     TEXT NOT NULL,
