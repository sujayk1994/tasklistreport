...
import { sendDailyReport, renderDailyReportHtml, sendPendingTasksEmail } from "../lib/email";
import { ensureTodayList, getLocalDateString } from "../lib/carryover";
import {
  triggerShipmentForCompletedPrint,
  parseTwitterMarketingTaskText,
  recordTwitterMarketingCompletion,
  parseReprintTaskText,
  recordReprintCompletion,
} from "../lib/inbox";
...
router.get("/tasks/today", requireAuth, async (req, res) => {
  ...
  if (!taskList) {
    res.json({ id: 0, date: today, submitted: false, submittedAt: null, checkedIn: false, checkedInAt: null, tasks: [] });
    return;
  }

  // Auto check-in: mark the user as checked in for today on their first load,
  // but only if they have autoCheckIn enabled AND today is a configured work day.
  // The auto-submit at 11:59 PM only fires for days where checkedIn is true,
  // so weekends and holidays (when autoCheckIn is off or not a work day) are skipped.
  if (!taskList.checkedIn) {
    const [userSettings] = await db
      .select()
      .from(userSettingsTable)
      .where(eq(userSettingsTable.userId, userId))
      .limit(1);
    const autoCheckIn = userSettings?.autoCheckIn ?? true;
    ...
    if (autoCheckIn && isWorkDay) {
      await db
        .update(taskListsTable)
        .set({ checkedIn: true, checkedInAt: new Date() })
        .where(eq(taskListsTable.id, taskList.id));
      taskList.checkedIn = true;
      taskList.checkedInAt = new Date();
    }
  }
  ...
  res.json({
    ...
    checkedIn: taskList.checkedIn,
    checkedInAt: taskList.checkedInAt?.toISOString() ?? null,
...
router.patch("/tasks/today/toggle", requireAuth, async (req, res) => {
  ...
  // If a Twitter Marketing task was just marked complete, record it so the
  // print-task scheduler can create a Print task 24 h later (once an address
  // receipt also exists for the same magazine/project).
  if (updated.completed && !task.completed) {
    const parsed = parseTwitterMarketingTaskText(updated.text);
    if (parsed) {
      recordTwitterMarketingCompletion(parsed.magazine, parsed.project).catch(
        () => {},
      );
    }
  }

  // If a Reprints task was just marked complete, record it for the project tracker.
  if (updated.completed && !task.completed) {
    const parsed = parseReprintTaskText(updated.text);
    if (parsed) {
      recordReprintCompletion(parsed.magazine, parsed.project).catch(() => {});
    }
  }
...
router.patch("/tasks/today/posted-for-future", requireAuth, async (req, res) => {
  ...
  const [updated] = await db
    .update(tasksTable)
    .set({ postedForFuture, ...(postedForFuture === false ? { remindDate: null } : {}) })
    .where(eq(tasksTable.id, taskId))
    .returning();
...
router.patch("/tasks/today/remind-date", requireAuth, async (req, res) => {
  const userId = getUserId(req);
  const { taskId, remindDate } = req.body as {
    taskId?: unknown;
    remindDate?: unknown;
  };
  const today = getLocalDateString();
  ...
  if (remindDate !== null && typeof remindDate !== "string") {
    res.status(400).json({ error: "remindDate must be a date string or null." });
    return;
  }

  if (typeof remindDate === "string" && !isValidDate(remindDate)) {
    res.status(400).json({ error: "remindDate must be a valid YYYY-MM-DD date." });
    return;
  }
  ...
  // Setting a remindDate files the task for future; clearing it restores it.
  const patch: Partial<typeof tasksTable.$inferInsert> = { remindDate };
  if (remindDate !== null) patch.postedForFuture = true;
...
// ---------------------------------------------------------------------------
// Manual check-in toggle — lets the user turn today's check-in on or off
// regardless of the autoCheckIn setting, so they can control whether the
// 11:59 PM auto-submit will fire for today.
// ---------------------------------------------------------------------------
router.patch("/tasks/today/checkin", requireAuth, async (req, res) => {
  const userId = getUserId(req);
  const today = getLocalDateString();
  const { checkedIn } = req.body as { checkedIn?: unknown };

  if (typeof checkedIn !== "boolean") {
    res.status(400).json({ error: "checkedIn must be a boolean." });
    return;
  }
  ...
  await db
    ...
    .set({
      checkedIn,
      checkedInAt: checkedIn ? (taskList.checkedInAt ?? new Date()) : null,
    })
    .where(eq(taskListsTable.id, taskList.id));
  ...
  res.json({
    ...
    checkedInAt: checkedIn
      ? (taskList.checkedInAt ?? new Date()).toISOString()
      : null,
  });
});
...
// Send pending tasks email on-demand (no submit, just email current pending tasks)
router.post("/tasks/send-pending-email", requireAuth, async (req, res) => {
  const userId = getUserId(req);
  const today = getLocalDateString();
  ...
  if (!recipientEmails.trim()) {
    res.status(400).json({ success: false, message: "No recipient emails configured. Add at least one address in Settings." });
    return;
  }
  ...
  if (!taskList) {
    res.status(400).json({ success: false, message: "No task list found for today." });
    return;
  }
  ...
  const tasks = await db
    .select()
    .from(tasksTable)
    .where(and(eq(tasksTable.taskListId, taskList.id), eq(tasksTable.completed, false), eq(tasksTable.postedForFuture, false)))
    .orderBy(tasksTable.position);

  if (tasks.length === 0) {
    res.status(400).json({ success: false, message: "No pending tasks to send." });
    return;
  }
  ...
  try {
    ...
    userName = user.fullName || user.emailAddresses[0]?.emailAddress || userId;
  } catch {
    // fall back to userId
  }
  ...
  const result = await sendPendingTasksEmail(
    recipientEmails,
    today,
    tasks.map((t) => ({ text: t.text, note: t.note ?? "" })),
    userName,
  );
