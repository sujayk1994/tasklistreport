...
export type BoardTask = {
  ...
  // When set, the task is scheduled to reappear on a specific date. It stays
  // in the Posted folder until that day, then activates automatically.
...
type Props = {
  ...
  // Schedule a task to reappear on a specific date (Reminder Calendar).
  onSetRemindDate?: (taskId: number, date: string | null) => void;
...
export function BoardView({
  ...
  // Which task currently has its calendar (reminder) picker open.
  const [openCalendarFor, setOpenCalendarFor] = useState<number | null>(null);
  // Which Posted-folder task is having its date changed inline.
  const [changingDateFor, setChangingDateFor] = useState<number | null>(null);
  ...
  // Close the per-card calendar picker when clicking anywhere outside of it.
  useEffect(() => {
    if (openCalendarFor === null) return;
    const onDown = () => setOpenCalendarFor(null);
    document.addEventListener("mousedown", onDown);
    return () => document.removeEventListener("mousedown", onDown);
  }, [openCalendarFor]);
      ...
      <div className="flex-1 min-h-0 relative">
        ...
        >
          ...
          >
            ...
            return (
              ...
              >
                <div
                  className="relative rounded-[14px] p-4 pb-4"
                  style={{
                      ...
                      {!isSubmitted && onSetRemindDate && (
                        <button
                          ...
                          style={{
                            ...
                            color: task.remindDate ? "#1D4ED8" : tag.ink,
                            opacity: task.remindDate ? 1 : 0.65,
                          }}
                        >
                          <Calendar size={11} />
                          {task.remindDate
                            ? new Date(task.remindDate + "T00:00:00").toLocaleDateString(undefined, { month: "short", day: "numeric" })
                            : ""}
                        </button>
                      )}
                      {openCalendarFor === task.id && (
                        <div
                          onMouseDown={(e) => e.stopPropagation()}
                          onClick={(e) => e.stopPropagation()}
                          className="absolute z-50 top-full left-0 mt-1 rounded-xl shadow-2xl border border-[#E2DBC6] bg-white p-3"
                          style={{ color: "#1F1B14", minWidth: 200 }}
                        >
                          <div className="text-[9px] uppercase tracking-[0.18em] font-semibold text-[#7A6F58] mb-2">
                            Remind me on
                          </div>
                          <input
                            type="date"
                            defaultValue={task.remindDate ?? ""}
                            min={new Date().toISOString().split("T")[0]}
                            className="w-full text-sm border border-[#E2DBC6] rounded px-2 py-1 mb-2 focus:outline-none focus:ring-2 focus:ring-[#1D4ED8]"
                            onMouseDown={(e) => e.stopPropagation()}
                            onChange={(e) => {
        ...
        {postedOpen && (
          ...
          >
            ...
            >
              Scheduled tasks reappear automatically on their reminder date.
            </div>
            <div className="flex-1 overflow-y-auto p-2">
              {postedTasks.length === 0 ? (
                ...
                <ul className="space-y-1.5">
                  {postedTasks.map((t) => (
                    <li
                      key={t.id}
                      className={`group flex flex-col gap-1 px-2.5 py-2 rounded-lg border border-transparent ${
                        darkMode
                    ...
                    >
                      ...
                      <div className="flex items-center gap-1.5 pl-5">
                        {t.remindDate ? (
                          <>
                            <Calendar size={11} style={{ color: darkMode ? "#93C5FD" : "#1D4ED8" }} />
                            <span className={`text-[11px] font-medium ${darkMode ? "text-[#93C5FD]" : "text-[#1D4ED8]"}`}>
                              {new Date(t.remindDate + "T00:00:00").toLocaleDateString(undefined, { month: "short", day: "numeric", year: "numeric" })}
                            </span>
                            {!isSubmitted && onSetRemindDate && (
                              <button
                                type="button"
                                onClick={() => setChangingDateFor((cur) => cur === t.id ? null : t.id)}
                                className={`text-[10px] px-1 py-0.5 rounded ${darkMode ? "text-[#9A9279] hover:text-[#BFDBFE]" : "text-[#7A6F58] hover:text-[#1D4ED8]"}`}
                          ...
                          !isSubmitted && onSetRemindDate && (
                            <button
                              type="button"
                              onClick={() => setChangingDateFor((cur) => cur === t.id ? null : t.id)}
                              className={`text-[10px] inline-flex items-center gap-1 px-1.5 py-0.5 rounded opacity-0 group-hover:opacity-70 hover:opacity-100 ${darkMode ? "text-[#9A9279]" : "text-[#7A6F58]"}`}
                              title="Add reminder date"
                            >
                      ...
                      {/* Inline date editor */}
                      {changingDateFor === t.id && onSetRemindDate && (
                        <div className="pl-5 flex items-center gap-1.5">
                          <input
                            type="date"
                            autoFocus
                            defaultValue={t.remindDate ?? ""}
                            min={new Date().toISOString().split("T")[0]}
                            className={`text-sm border rounded px-2 py-0.5 focus:outline-none focus:ring-2 focus:ring-[#1D4ED8] ${darkMode ? "bg-[#1f1c19] border-[#3a3530] text-[#E5E0D6]" : "bg-white border-[#E2DBC6] text-[#1F1B14]"}`}
                            onChange={(e) => {
                              const date = e.target.value;
                              if (date) {
                                onSetRemindDate(t.id, date);
                                setChangingDateFor(null);
                              }
                            }}
                            onBlur={() => setChangingDateFor(null)}
                          />
                          {t.remindDate && (
                            <button
                              type="button"
                              ...
                              onClick={() => {
                                onSetRemindDate(t.id, null);
                                setChangingDateFor(null);
                              }}
                              className={`text-[10px] px-1.5 py-0.5 rounded ${darkMode ? "text-[#9A9279] hover:text-[#E5E0D6]" : "text-[#7A6F58] hover:text-[#1F1B14]"}`}
                            >
                              Clear
                            </button>
                          )}
